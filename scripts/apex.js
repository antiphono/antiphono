...
(function () {
  ...
  /* =========================================================
     /index2 choreography. Every one of these is a no-op on a
     page without its markup, and every one collapses under
     prefers-reduced-motion. Nothing here listens to the scroll
     event: ScrollTrigger and pointer events only.
     ========================================================= */

  /* Hero lines rise out of their own overflow on load. Communicates
     hierarchy: the headline arrives before anything under it. */
  function heroLines() {
    var lines = document.querySelectorAll('[data-hero-line]');
    if (!lines.length) return;
    if (reduced || !hasGsap) {
      Array.prototype.forEach.call(lines, function (l) { l.style.transform = 'none'; });
      return;
    }
    gsap.set(lines, { yPercent: 108 });
    gsap.to(lines, {
      yPercent: 0, duration: 1.1, ease: 'expo.out', stagger: 0.075, delay: 0.15
    });
  }
  ...
  /* Buttons lean towards the cursor. Feedback: the control
     ...
     to the transform, never through state, and pointer devices
     only. */
  function magnetic() {
